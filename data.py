...
class MyClass:
    ...
    def select_country(self, country, **kwargs):
        self._read_data()
        df = self.df[(self.df['Country/Region'] == country) & ((self.df['Province/State'].isna()) | (self.df['Province/State'] == country))]
        return df.iloc[0, 44:]

    def plot(self):
        countries = ['Austria', 'Italy', "Germany", "Sweden", "Spain", "Greece", "US"]

        data = [go.Scatter(x=self.df.columns[44:],
                           y=self.select_country(country),
                           name=country) for country in countries]


        layout = go.Layout(
            title='Covid 19 Cases',
            yaxis=dict(title='New infections each day'),
